    ...
    const onSubmitHandler = async(e)=>{
        try {
            ...
            if(!name || !description || !category || !price || !offerPrice || files.length === 0){
                return toast.error("Fill all the product details")
            }
           ...
           const {data} = await axiosReq.post('/api/product/add',formData);
           if(data.success){
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img className='max-w-24 cursor-pointer' src={files[index] ? URL.createObjectURL(files[index]) : assets.upload_area} width={100} height={100} alt="upload Area" />
                            </label>
                        ))}
                    </div>
                </div>
